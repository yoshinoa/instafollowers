...
def main():
    ...
    parser.add_argument('-u', '--username', help="Set the username to run with.",
                        action='store', dest='username', required=True)
    parser.add_argument(
        '-p', '--password', help="Set the password to run with.", action='store', dest='password')
    parser.add_argument(
        '-t', '--target', help="Set the target to run against.", action='store', dest='target')
    parser.add_argument(
        '-o', '--output', help="Set the output file to write to.", action='store', dest='output')
    parser.add_argument('-c', '--compare', help="Compare an old pickle to a new username, input the revision number or 0 for no revision number",
                        action='store', dest='compare')
    parser.add_argument('-l', '--local', help="Load a pickle file",
                        action='store', dest='local')
    parser.add_argument(
        '-date', '--date', help="Check the dates of the pickle files currently stored", action='store_true', dest='date')
            ...
            for file in os.listdir('pickles'):
                if file.endswith('.pickle'):
                    with open(f'pickles/{file}', 'rb') as f:
                        print(
                            f'File: {file} - Date: {pickle.load(f).updated_at}')
    if args.setup:
    ...
    if args.compare and args.target:
        with open(f'pickles/{args.target}{"_" + args.compare if args.compare != 0 else ""}.pickle', 'rb') as f:
            old = pickle.load(f)
        # look at pickle info
        if args.local:
            with open(f'pickles/{args.target}{"_" + args.local if args.local != 0 else ""}.pickle', 'rb') as f:
